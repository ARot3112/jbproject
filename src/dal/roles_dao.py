...
#     def insert_into_roles(self,name):
#         with jb_db_conn.cursor() as cur:
#             cur.execute(psycopg.sql.SQL("INSERT INTO {} (name) VALUES (%s);").format(psycopg.sql.Identifier(self.table_name)),(name,))
#             jb_db_conn.commit()


#     def get_roles_info_by_id(self,id):
#         with jb_db_conn.cursor(row_factory=pgrows.dict_row) as cur:
#             cur.execute(psycopg.sql.SQL("SELECT * FROM {} WHERE id = %s;").format(psycopg.sql.Identifier(self.table_name)),(id,))
#             result = cur.fetchall()
#         print(result)

#     def update_roles_info_by_id(self,id,column,new_value):
#         with jb_db_conn.cursor() as cur:
...
#     def delete_roles_info_by_id(self,id):
#         with jb_db_conn.cursor() as cur:
